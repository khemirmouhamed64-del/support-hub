...
<body>
    ...
    <aside class="sidebar">
        <div class="brand">
            <i class="fas fa-headset"></i> {{ __('app.brand_name') }}
            <small>{{ __('app.brand_subtitle') }}</small>
        </div>
        <nav class="mt-2">
            <a href="{{ url('/dashboard') }}" class="nav-link {{ request()->is('dashboard') ? 'active' : '' }}">
                <i class="fas fa-columns"></i> {{ __('app.nav_kanban') }}
            </a>
            <a href="{{ url('/tickets') }}" class="nav-link {{ request()->is('tickets*') ? 'active' : '' }}">
                <i class="fas fa-ticket-alt"></i> {{ __('app.nav_tickets') }}
            </a>
            <a href="{{ url('/clients') }}" class="nav-link {{ request()->is('clients*') ? 'active' : '' }}">
                <i class="fas fa-building"></i> {{ __('app.nav_clients') }}
            </a>
            <a href="{{ url('/team') }}" class="nav-link {{ request()->is('team*') ? 'active' : '' }}">
                <i class="fas fa-users"></i> {{ __('app.nav_team') }}
            </a>
            <a href="{{ url('/reports') }}" class="nav-link {{ request()->is('reports*') ? 'active' : '' }}">
                <i class="fas fa-chart-bar"></i> {{ __('app.nav_reports') }}
            </a>
            <a href="{{ url('/tickets/archived') }}" class="nav-link {{ request()->is('tickets/archived') ? 'active' : '' }}" style="margin-top: 15px; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 15px;">
                <i class="fas fa-archive"></i> {{ __('app.nav_archived') }}
            </a>
        </nav>
    </aside>
    ...
    <div class="main-content">
        ...
        <div class="top-navbar">
            <h5 class="mb-0">@yield('page-title', __('app.dashboard'))</h5>
            <div class="d-flex align-items-center">
                ...
                <div class="dropdown mr-3" id="notifications-dropdown">
                    <a href="#" class="text-dark position-relative" data-toggle="dropdown">
                        ...
                        <span class="badge badge-danger notification-badge" id="notif-badge" style="{{ $unreadCount > 0 ? '' : 'display:none;' }}">{{ $unreadCount }}</span>
                    </a>
                    <div class="dropdown-menu dropdown-menu-right p-0" style="width: 320px; max-height: 420px; overflow-y: auto;">
                        <div class="d-flex justify-content-between align-items-center px-3 py-2 border-bottom">
                            ...
                            <div>
                                <button type="button" class="btn btn-link btn-sm p-0 text-muted mr-2" id="btn-toggle-read" title="{{ __('app.show_read_notifs') }}">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <button type="button" class="btn btn-link btn-sm p-0 text-muted" id="btn-mark-all-read" title="{{ __('app.mark_all_read') }}">
                        ...
                        @forelse($allNotifs as $notif)
                            <div class="notif-item px-3 py-2 border-bottom {{ $notif->read_at ? 'notif-read notif-hidden' : 'bg-light' }}"
                                 data-id="{{ $notif->id }}" data-ticket-id="{{ $notif->ticket_id }}" style="cursor:pointer;">
                                <div class="flex-grow-1">
                                    <strong class="small">{{ $notif->title }}</strong>
                                    @if($notif->message)<br><span class="small text-muted">{{ Str::limit($notif->message, 60) }}</span>@endif
                        ...
                        @empty
                            <div id="notif-empty" class="px-3 py-3 text-center text-muted small">{{ __('app.no_notifications') }}</div>
                        @endforelse
                        @if($allNotifs->isNotEmpty() && $allNotifs->whereNull('read_at')->isEmpty())
                            <div id="notif-all-read-msg" class="px-3 py-3 text-center text-muted small">{{ __('app.all_caught_up') }}</div>
                        @else
                ...
                <div class="dropdown">
                    <a href="#" class="d-flex align-items-center text-dark text-decoration-none" data-toggle="dropdown">
                        ...
                        <span class="badge badge-secondary">{{ strtoupper(auth()->user()->role) }}</span>
                    </a>
                    <div class="dropdown-menu dropdown-menu-right">
                        <a class="dropdown-item" href="{{ url('/profile') }}"><i class="fas fa-user mr-2"></i>{{ __('app.profile') }}</a>
                        <div class="dropdown-divider"></div>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="dropdown-item"><i class="fas fa-sign-out-alt mr-2"></i>{{ __('app.logout') }}</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    {{-- Mention suggestions container (global, repositioned by JS) --}}
        ...
        (function() {
            ...
            $('#btn-toggle-read').on('click', function(e) {
                ...
                if (showingRead) {
                    ...
                    $(this).attr('title', '{{ __('app.hide_read_notifs') }}');
                    $('.notif-read').removeClass('notif-hidden');
                } else {
                    $(this).html('<i class="fas fa-eye"></i>');
                    $(this).attr('title', '{{ __('app.show_read_notifs') }}');
